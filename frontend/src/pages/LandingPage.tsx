...
import type { User } from "@/types/auth.types"; // Importar Tipo
...
const LandingPage = () => {
    ...
    // ESTADO NUEVO: Para guardar al usuario que encontramos en el backend
    ...
    // Función para cancelar (Botón "Volver" o "No soy yo")
    const handleVolver = () => {
        logout(); // Limpiamos sesión por seguridad
    ...
    return (
        <div className="min-h-screen w-full bg-gray-50 flex flex-col overflow-hidden relative">
            ...
            <div className="w-full h-20 bg-white shadow-sm flex items-center px-6 md:px-12 z-10 flex-shrink-0">
                <img src={iconOnpe} className="h-12 w-auto cursor-pointer hover:opacity-80 transition-opacity" alt="Logo ONPE" />
            </div>
            ...
            <div className="flex-1 flex flex-col md:flex-row justify-center w-full max-w-[1600px] mx-auto p-6 md:px-12 lg:px-20 gap-10 lg:gap-24 pt-8 md:pt-16 lg:pt-20">
                ...
                {/* COLUMNA IZQUIERDA (CARRUSEL - Sin cambios) */}
                <div className="w-full md:w-1/2 flex justify-center md:justify-end items-start animate-fadeIn">
                    <div className="relative w-full max-w-[350px] md:max-w-[500px] lg:max-w-[600px] aspect-square">
                        <img src={images[index]} alt="Campaña" className="w-full h-full object-contain rounded-3xl shadow-2xl transition-all duration-700 hover:scale-[1.02]" />
                    ...
                    {pasoActual === 'inicio' && (
                        <div className="flex flex-col items-center md:items-start text-center md:text-left w-full max-w-2xl animate-fadeIn">
                            ...
                            <div className="flex items-center justify-center md:justify-start w-full mb-8">
                                <img src={iconCheck} alt="Check icon" className="w-20 md:w-24 lg:w-28" />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full sm:w-2/3 md:w-auto md:px-16 py-4 lg:py-5 bg-blue-950 text-white text-xl lg:text-2xl font-semibold rounded-2xl shadow-xl hover:bg-blue-900 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300"
                            >
                                INICIAR VOTO
                            </button>
                        </div>
                    ...
                    {pasoActual === 'dni' && (
                        <div className="w-full max-w-lg lg:max-w-xl mx-auto md:mx-0 animate-fadeIn mt-4 lg:mt-8">
                            <DniPanel
                                ...
                                // Cuando el login es exitoso, guardamos el user y avanzamos
                                onUsuarioEncontrado={(user) => {
                                    setUsuarioEncontrado(user);
                                    setPasoActual('DniInfo');
                                }}
                            />
                        </div>
                    )}

                    {/* VISTA: INFO DNI (DniInfoPanel) */}
                    {pasoActual === 'DniInfo' && usuarioEncontrado && (
                        <div className="w-full max-w-lg lg:max-w-xl mx-auto md:mx-0 animate-fadeIn mt-4 lg:mt-8">
                            <DniInfoPanel
                                user={usuarioEncontrado} // Le pasamos el usuario directamente
                                onVolver={handleVolver}
                            />
                        </div>
                    )}
            ...
            <button onClick={() => setShowModal(true)} className="fixed bottom-8 left-8 w-16 h-16 lg:w-20 lg:h-20 bg-blue-950 text-white rounded-full shadow-2xl flex items-center justify-center text-3xl lg:text-4xl font-bold z-40 hover:scale-110 hover:bg-blue-800 transition-all duration-300">?</button>
            <button onClick={() => setShowChatIA(!showChatIA)} className="fixed bottom-8 right-8 w-16 h-16 lg:w-20 lg:h-20 bg-blue-950 text-white rounded-full shadow-2xl flex items-center justify-center text-2xl lg:text-3xl font-bold z-40 hover:scale-110 hover:bg-blue-800 transition-all duration-300">💬</button>
