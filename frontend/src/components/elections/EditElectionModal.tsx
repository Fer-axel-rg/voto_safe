...
// src/components/elections/EditElectionModal.tsx
...
interface EditElectionModalProps {
  ...
  onClose: () => void;
  onSave: (id: string, election: Election) => void;
}
...
export default function EditElectionModal({
  ...
  useEffect(() => {
    if (election) {
      setFormData({
        ...
        allowNullVote: election.allowNullVote,
        requireMinimumCategory: election.requireMinimumCategory,
        allowMultipleVotes: election.allowMultipleVotes,
        autoSendVote: election.autoSendVote,
      });
      setCategories(
  ...
  const handleSubmit = () => {
    ...
    onSave(election.id, updatedElection);
    handleClose();
  };
